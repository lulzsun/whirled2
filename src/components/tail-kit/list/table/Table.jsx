...
const Table = (props) => {
    ...
    return (<table className="table p-4 bg-white shadow rounded-lg">
            <thead>
                <tr>
                    {headers.map((head) => {
            return (<th key={head} className={`${borderClasses} p-4 dark:border-dark-5 whitespace-nowrap font-normal text-gray-900`}>
                                {head}
                            </th>);
        })}
                </tr>
            ...
            return (<tr className="text-gray-700" key={index}>
                            {row.data.map((text) => {
                    return (<td key={text} className={`${borderClasses} p-4 dark:border-dark-5`}>
                                        {text}
                                    </td>);
                })}
                        </tr>);
