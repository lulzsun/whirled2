...
const BlogList = (props) => {
    ...
    return (<div className="w-full bg-white p-12">
            <div className="header flex items-end justify-between mb-12">
                ...
                {props.withSearch && (<div className="text-end">
                        <FormSubscribe label="Search" placeholder="Enter a title"/>
                    </div>)}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-12">
                {blogs.map((blog) => {
            return (<BlogCard key={blog.title} tags={blog.tags} title={blog.title} categ={blog.categ} img={blog.img} desc={blog.desc} showAuthor={true}/>);
        })}
            </div>
        </div>);
};
