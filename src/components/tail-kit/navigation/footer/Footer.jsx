...
const Footer = (props) => {
    return (<footer className={`bg-white dark:bg-gray-800 pt-4 pb-8 xl:pt-8`}>
            ...
            return (<li key={link.label} className="w-1/2 md:w-1/3 lg:w-1/3">
                                <div className="text-center">
                                    <h2 className={`text-gray-500 dark:text-gray-200 text-md uppercase mb-4`}>
                                        {link.label}
                                    </h2>
                                    {link.subLinks && props.showSubLinks && (<ul>
                                            {link.subLinks.map((sub) => {
                        return (<li className="mb-4 hover:text-gray-800 dark:hover:text-white transition-colors duration-200" key={sub}>
                                                        <a href="#">{sub}</a>
                                                    </li>);
                    })}
                                        </ul>)}
                ...
                {props.withSocialLink && (<div className={`pt-8 flex border-t border-gray-200 max-w-xs mx-auto items-center justify-between`}>
                        <a href="#">
                ...
                {props.withCredits && (<div className={`text-center pt-10 sm:pt-12 font-light flex items-center justify-center`}>
                        Created by Charlie
                    </div>)}
            </div>
        </footer>);
